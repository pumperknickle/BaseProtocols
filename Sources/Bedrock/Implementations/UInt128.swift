import Foundation

public struct UInt128: Codable {
    public var parts: [UInt64]
    
    public init() {
        self.init(withUInt64: 0)
    }
    
    public init(_ value: Int) {
        if value >= 0 {
            parts = [0, UInt64(value)]
        } else {
            parts = [
                UInt64.max,
                UInt64(truncatingIfNeeded: value),
            ]
        }
    }
    
    
    public init(_ value: UInt64) {
        self.init(withUInt64: value)
    }
    
    public init(withUInt64 value: UInt64) {
        parts = [0, value]
    }
    
    public init(_ value: UInt128) {
        parts = [value[0], value[1]]
    }
    
    public init(_ values: [UInt64]) {
        self.init(withUInt64Array: values)
    }
    
    public init(withUInt64Array values: [UInt64]) {
        var _parts = Array(values)
        while _parts.count < 2 {
            _parts.insert(0, at: 0)
        }
        parts = Array(_parts[0 ... 1])
    }
    
    public subscript(index: Int) -> UInt64 {
        get {
            return parts[index]
        }
        mutating set {
            parts[index] = UInt64(newValue)
        }
    }
    
    public subscript(range: CountableClosedRange<Int>) -> [UInt64] {
        return Array(parts[range])
    }
    
    public func toHexString() -> String {
        return String(format: "%0.16lx%0.16lx", parts[0], parts[1])
    }
}
